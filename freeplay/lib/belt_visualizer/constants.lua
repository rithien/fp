...
local function empty_offsets(splitter)
    local function fresh()
        return splitter and { left = {}, right = {} } or {}
    end
    return { {
        [defines.direction.north] = fresh(),
        [defines.direction.east] = fresh(),
        [defines.direction.south] = fresh(),
        [defines.direction.west] = fresh(),
    }, {
        [defines.direction.north] = fresh(),
        [defines.direction.east] = fresh(),
        [defines.direction.south] = fresh(),
...
local function rectangle(base_offsets, data)
    local t = empty_offsets()
    for name, v in pairs(data) do
        local offset_name = v[1]
        local size = v[2]
        for lane, directions in pairs(base_offsets) do
            for direction, lane_offsets in pairs(directions) do
                local o = lane_offsets[offset_name]
                t[lane][direction][name] = {
                    left_top = { o[1] - size, o[2] - size },
                    right_bottom = { o[1] + size, o[2] + size },
                }
            end
        end
    end
