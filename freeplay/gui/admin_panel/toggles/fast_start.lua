...
local ROBOTS_COUNT = 25
local function ensure_storage()
    if not storage.fast_start_given then
        storage.fast_start_given = {}
    end
end
local function has_received(player)
    ensure_storage()
    return storage.fast_start_given[player.name] == true
end
local function mark_received(player)
    ensure_storage()
    storage.fast_start_given[player.name] = true
end
...
AdminPanel.register_toggle({
    id = TOGGLE_ID,
    caption = { 'fp-admin.fast-start-caption' },
    tooltip = { 'fp-admin.fast-start-tooltip' },
    get_state = function() return Config.is_enabled(TOGGLE_ID) end,
    apply = function(state)
        if state then
            Config.set('fast_start_mech', false)
            give_to_all_online()
        end
    end,
    ...
    for _, player in pairs(game.connected_players) do
        if player.valid and player.character and player.character.valid then
            ...
            if armor_inv and armor_inv.valid and armor_inv.is_empty() then
                storage.fast_start_given[player.name] = nil 
                if give_to_player(player) then
                    count = count + 1
                end
            end
